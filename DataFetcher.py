                    ...
                    if len(data["MRData"]["StandingsTable"]["StandingsLists"]) > 0:
                        ...
                        for standing in data["MRData"]["StandingsTable"]["StandingsLists"][0]["DriverStandings"]:
                            ...
                            try:
                                ...
                                self.cur.execute(
                                    "SELECT driverid FROM drivers where drivers.url = %s AND drivers.forename = %s ",
                                    (str(standing["Driver"]["url"]), str(standing["Driver"]["givenName"])))
                                driver_id = self.cur.fetchone()[0]

                                self.cur.execute("INSERT INTO driverstandings(raceid,driverid"
                                                 ",points,position,positiontext,wins) VALUES (%s,%s,%s,%s,"
                                                 "%s,%s)", (
                                                     race_id, driver_id, standing["points"], standing["position"],
                                                     standing["positionText"], standing["wins"]))
                                count1 += 1
                                print(standing["Driver"]["url"])
                                self.conn.commit()
...
#http://ergast.com/api/f1/2023/7/laps.json?limit=2000
    ...
    def fetchPitstops(self):
        ...
        print(years[0][0])
        for year in years:
            # http://ergast.com/api/f1/2011/5/pitstops.json?limit=2000
            self.cur.execute(
                "SELECT max(round) FROM races inner join Seasons on races.year = Seasons.year WHERE races.year = %s",
                (year,))
            maxRound = int(self.cur.fetchone()[0])
            ...
            for round in range(1, maxRound):
                ...
                with urllib.request.urlopen(
                        "http://ergast.com/api/f1/" + str(year[0]) + "/" + str(
                            round) + "/pitstops.json?limit=2000") as url:
                    data = json.load(url)

                    if len(data["MRData"]["RaceTable"]["Races"]) > 0:

                        for stop in data["MRData"]["RaceTable"]["Races"][0]["PitStops"]:
